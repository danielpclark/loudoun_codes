...
RSpec.describe SubmissionRunners::Elixir, type: 'docker' do
  describe 'docker command and barebone Elixir image' do
    let(:fixtures) { Pathname.new(Rails.root).join('spec/fixtures/submission_runners/elixir') }
    ...
    let(:submission) {
      Submission.create_from_file({
        problem:  problem,
        account:  account,
        filename: fixtures.join("#{problem_name}.exs"),
      }).tap(&:validate!)
    }
